...
class Routing {
    ...
    public function run() {
        //where is project exacly ##### error
        $path = realpath(dirname(__FILE__) . "/../../application/controlles" . $this->current_route[0] . "php");
        if(file_exists($path)) {
        ...
        //just one cell method = index ##### error
        sizeof($this->current_route) == 1 ? $method = "index" : $method = $this->current_route[1];
        $class = "Application\controlles\\" . $this->current_route[0];
        $object = new $class();
